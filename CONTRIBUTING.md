...
We use different branch types for different kinds of work.

  -----------------------------------------------------------------------------------
  Branch                  Purpose                     Example
  ----------------------- --------------------------- -------------------------------
  `main`                  Stable, production-ready    `main`
                          code only. Never push       
                          directly.                   

  `develop`               Integration branch for      `develop`
                          completed features and      
                          fixes before release.       

  `feature/*`             New functionality or        `feature/user-authentication`
                          enhancements.               

  `bugfix/*`              Fixes for bugs discovered   `bugfix/login-validation`
                          during normal development.  

  `hotfix/*`              Urgent fixes for the stable `hotfix/payment-crash`
                          `main` branch.              

  `refactor/*`            Code restructuring without  `refactor/api-service`
                          changing intended behavior. 

  `docs/*`                Documentation-only changes. `docs/contributing-guide`

  `test/*`                Adding or improving tests.  `test/auth-unit-tests`

  `chore/*`               Maintenance tasks such as   `chore/update-dependencies`
                          dependency/config/tooling   
                          updates.                    
  -----------------------------------------------------------------------------------
...
A hotfix should be merged back into both `main` and `develop` when
applicable, so the fix does not disappear from future development.
...
Use `hotfix/*` only for urgent problems affecting the stable `main`
branch.
...
git push -u origin hotfix/critical-login-error
```

Open a PR targeting `main`.

After the hotfix is merged into `main`, make sure the same fix is also
present in `develop`.
